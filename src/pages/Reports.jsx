...
const Reports = () => {
  ...
  // Fetch user role from /api/users/me
  useEffect(() => {
    const fetchUserRole = async () => {
      try {
        ...
        setUserRole('User'); // Fallback to User role to hide buttons
      }
    };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-all duration-300">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="p-4 bg-gray-50 rounded-xl border border-gray-100 hover:bg-white hover:border-blue-500 transition-all duration-300">
              <p className="text-sm text-gray-600">Your Total Opportunities</p>
              <p className="text-2xl font-bold text-gray-800 mt-2">{keyIndicators.totalOpportunitiesForUser}</p>
            </div>
            <div className="p-4 bg-gray-50 rounded-xl border border-gray-100 hover:bg-white hover:border-blue-500 transition-all duration-300">
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {userPerformance ? (
                <tr className="hover:bg-gray-50 transition-all duration-200">
                  <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-800">{userPerformance.name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-600">{parseFloat(userPerformance.target).toLocaleString()} TND</td>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-600">{parseFloat(userPerformance.achieved).toLocaleString()} TND</td>
                  <td className="px-6 py-4">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2.5">
                          <div
                            className={`h-2.5 rounded-full ${
                              userPerformance.progress >= 100 
                                ? 'bg-green-500' 
                                : userPerformance.progress >= 70 
                                ? 'bg-blue-500' 
                                : userPerformance.progress >= 40 
                                ? 'bg-yellow-500' 
                                : 'bg-red-500'
                            }`}
                            style={{ width: `${Math.min(userPerformance.progress, 100)}%` }}
                          ></div>
                        </div>
                      </div>
                ...
                <tr>
                  <td colSpan="4" className="px-6 py-10 text-center text-gray-600">
                    ...
                    <p>No performance data available for the current user</p>
                  </td>
                </tr>
              )}
            </tbody>
